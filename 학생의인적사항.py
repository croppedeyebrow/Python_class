...
# 저장은 Rest API의 기본 형태인 JSNO으로 저장 및 불러오기. (웹에서 API의 기본 포멧)
# 함수로 구현
import json  # JSON 형식으로 데이터를 저정하고 불러오기 위해 json 모듈을 import
...
def reg_student() :
    ...
    #학생 정보 탐색
def search_student() :
    student_id = input("검색 할 학번을 입력 : ")
    student_info = student_map.get(student_id) # get(key) 매서드로 학번을 얻어 옴.
    if student_info :
...
def modify_student() :
    ...
    student_info = student_map.get(student_id) # 학번을 키로 해서 해당하는 값(이름과 주소로 구성된 딕셔너릴)
    if student_info :
